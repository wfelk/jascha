...
const Date: FC<Props> = ({ dateTime, date, month, year }) => {
  return (
    ...
    >
      ...
      <span className="text-5xl md:text-3xl text-sky-100/90">{date}</span>
      <span className="text-2xl md:text-xl text-sky-300">{month}</span>
      <span className="text-4xl md:text-2xl text-teal-300/50">{year}</span>
    </time>
  );
};
